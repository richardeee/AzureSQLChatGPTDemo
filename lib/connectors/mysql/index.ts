...
const convertToConnectionUrl = (connection: Connection): string => {
  ...
  console.log(`mysql://${connection.username}:${connection.password}@${connection.host}:${connection.port}/${connection.database ?? ""}`)
...
const getMySQLConnection = async (connection: Connection): Promise<mysql.Connection> => {
  const conn = await mysql.createConnection({
    ...
    debug:true,
    ssl: {
      ca: fs.readFileSync(path.join(__dirname, "/cert/DigiCertGlobalRootCA.crt.pem"))
    }
  });
  return conn;
};
